<template>
  <canvas id="graph-canvas" width="750" height="350"/>
</template>
...
let inputData = ref([
  ...
  {time: '2022-01-02T00:00:00Z', value: 15},
  {time: '2022-01-03T00:00:00Z', value: 30},
  {time: '2022-01-04T00:00:00Z', value: 20},
  {time: '2022-01-05T00:00:00Z', value: 25},
  {time: '2022-01-06T00:00:00Z', value: 35},
  {time: '2022-01-07T00:00:00Z', value: 40},
  {time: '2022-01-08T00:00:00Z', value: 50},
  {time: '2022-01-09T00:00:00Z', value: 45}
]);
...
const getPointCoords = computed(() => {
  ...
  for (let i = 0; i < inputData.value.length; i++) {
    outputCoords.push({
      x: i * ((width * 0.9) / inputData.value.length),
...
function drawGraph() {
  ...
  for (let i = 0; i <= numGridLines; i++) {
    ...
    ctx.fillText(label.toString(), 50, height - i * pixelsPerVerticalGridline + 5);
  }

  // Draw data line
  ctx.beginPath();
  ctx.strokeStyle = '#26DCB7';
  ctx.lineWidth = 4;
  ...
  ctx.beginPath();
  ctx.arc(getPointCoords.value[getPointCoords.value.length - 1].x, getPointCoords.value[getPointCoords.value.length - 1].y, 4, 0, 2 * Math.PI);
  ...
  for (let i = 0; i < dataLength; i++) {
    if (i % labelInterval === 0 || i === dataLength - 1) {
      const xPos = (width * 0.9) * (i / (dataLength - 1));
